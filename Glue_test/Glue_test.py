...
try:
	...
	df = spark.read.format('csv').options(header='True').load('s3://dataops-source-bucket/Noneus-500.csv')
	...
	#Validation-notempty
	df = df.filter(~col('city').isNull()).limit()
	#Transformations
	else:
	...
	t_s3_credentials = client.read('dataops/data/data/dataops-source-bucket')['data']['data']
	...
	df.write.mode('overwrite').format('csv').save('s3a://dataops-source-bucket/None/None')
