...
try:
	...
	df = spark.read.format('csv').options(header='True').load('s3://test-repo-buckets/us-500.csv')
	...
	df.write.mode('overwrite').format('avro').save('s3a://pink-buckets/one/')
...
s3.upload_file('audit_logs.csv', 'pink-buckets', 'logs/audit_logs.csv')
