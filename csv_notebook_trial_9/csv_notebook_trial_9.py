...
try:
	...
	df = spark.read.format('json').options(header='True').load('s3://red-buckets/us-500.json')
	...
	df.write.mode('overwrite').format('parquet').save('s3a://blue-buckets/json_to_parquet/')
