...
try:
	...
	df = spark.read.format('parquet').options(header='True').load('s3://red-buckets/us-500.parquet')
	...
	df.write.mode('overwrite').format('avro').save('s3a://blue-buckets/parquet_to_avro/')
