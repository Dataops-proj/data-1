...
try:
	...
	df.write.format('jdbc').mode('overwrite').option('url', '').option('driver', 'org.mariadb.jdbc.Driver').option('dbtable', 'postgres_to_mariadb').option('user', username_t).option('password', password_t).save()
