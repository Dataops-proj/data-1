...
try:
	...
	df = spark.read.format('jdbc').option('url', 'jdbc:postgres://dataops-db.cr5bcibr4zvb.ap-south-1.rds.amazonaws.com:5432/postgres').option('driver', 'org.postgresql.Driver,com.mysql.cj.jdbc.Driver,org.mariadb.jdbc.Driver,com.microsoft.sqlserver.jdbc.SQLServerDriver,oracle.jdbc.OracleDriver').option('dbtable', 'us').option('user', username_s).option('password', password_s).load()
