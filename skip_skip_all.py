...
try:
	...
	# Validation-notempty (Stop)
	df = df.filter(~col('first_name').isNull()).limit(100)
	if df.count() == 0:
		raise ValueError('Notempty validation failed. Stopping processing.')

	# Validation-notempty (Skip)
	df = df.filter(~col('last_name').isNull()).limit(50)

	# Validation-custom (Stop)
	if df.filter(df['company_name'].rlike('@')).count() > 0:
		raise ValueError('Custom validation (Stop) failed. Stopping processing.')
